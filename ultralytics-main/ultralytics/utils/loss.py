...
class BboxLoss(nn.Module):
    ...
    def forward(self, pred_dist, pred_bboxes, anchor_points, target_bboxes, target_scores, target_scores_sum, fg_mask):
        ...
        iou_kwargs = {'xywh': False, self.iou_type: True, 'Inner': self.inner_iou, 'ratio': self.inner_ratio,
                       'focaler_d': self.focaler_d, 'focaler_u': self.focaler_u}
...
class v8DetectionLoss:
    ...
    def __init__(self, model, tal_topk=10):  # model must be de-paralleled
        ...
        self.bbox_loss = BboxLoss(m.reg_max, iou_type=getattr(h, 'iou_type', 'CIoU'), inner_iou=getattr(h, 'inner_iou', False), inner_ratio=getattr(h, 'inner_ratio', 0.7), focaler_d=getattr(h, 'focaler_d', 0.0), focaler_u=getattr(h, 'focaler_u', 0.95)).to(device)
